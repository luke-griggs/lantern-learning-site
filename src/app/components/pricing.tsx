...
const pricingOptions: Record<string, PricingPlans> = {
  kthrough8: {
    thirtyMinute: {
      ...
      features: [],
      link: "https://calendly.com/lanternlearning/30-minute-virtual-tutoring-session"
    },
    sixtyMinute: {
      type: "60 min",
      price: "$44.99",
      ...
      note: "flexible scheduling",
      features: [],
      link: "https://calendly.com/lanternlearning/60min"
    },
    group: {
      type: "Group",
      price: "$29.99",
      ...
      features: [],
      link:"https://calendly.com/lanternlearning/30-minute-virtual-tutoring-session-group-session",
    },
  },
  highschool: {
    thirtyMinute: {
      ...
      note: "flexible scheduling",
      features: [],
      link:"https://calendly.com/lanternlearning/30-minute-virtual-tutoring-session-high-school",
    },
    sixtyMinute: {
      type: "60 min",
      price: "$54.99",
      description: "/ month",
      note: "flexible scheduling",
      link:"https://calendly.com/lanternlearning/60-minute-online-tutoring-session-high-school",
...
const Pricing = () => {
  const [activeTab, setActiveTab] =
    useState<keyof typeof pricingOptions>("kthrough8");
  ...
  const renderPricingOption = (option: PricingOption) => (
    <>
      <div className="bg-white rounded-lg shadow-lg sm:min-w-[300px]">
        ...
        <div className="w-full bg-[#62564F] h-0.5"></div>
        <div className="p-6">
          <h2 className="text-3xl font-bold text-[#30231B]">
            {option.price}
            <span className="text-gray-500 text-sm font-normal">/session</span>
          </h2>
          {option.note && (
            <p className="text-sm text-gray-500 mt-2">{option.note}</p>
          )}
  ...
  return (
    <div className="flex flex-col items-center p-16 text-center">
      ...
      <p className="text-[#62564F] mb-6">
        All sessions take place via Zoom. If you have any additional questions
        regarding pricing, please feel free to{" "}
        <a
          href="mailto:lukegriggs100@gmail.com"
      ...
      <div className="flex mb-8 border-[#30231B] border-2 rounded-lg">
        {Object.keys(pricingOptions).map((tab) => (
          <button
            key={tab}
            className={`py-2 px-4 ${
              activeTab === tab ? "bg-[#30231B] text-white" : "text-[#30231B] duration-300"
            }`}
